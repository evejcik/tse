...
static void crawl(char* seedURL, char* pageDirectory, const int maxDepth){
    ...
    //hput(visitedURLs, seedURL, seedURL, strlen(seedURL)); //put seedURL into the hashtable
    ...
    if (hput(visitedURLs, webpage_getURL(firstURL), webpage_getURL(firstURL), strlen(webpage_getURL(firstURL))) != 0){ // insert into hashtable
    ...
    if(visitedURLs != NULL){
      ...
      hclose(visitedURLs); //deletes hashtable of visited URLs.
    }
    fprintf(stdout, "Finished hclose.\n");
    if(pagesToCrawl != NULL){
      fprintf(stdout, "pagesToCrawl is not NULL.\n");
      qclose(pagesToCrawl); //deletes queue of webpages.
    }
...
static void pageScan(webpage_t* page, queue_t* pagesToCrawl, hashtable_t* visitedURLs){
    ...
    while ((pos = webpage_getNextURL(page, pos, &result)) > 0){ // while there is another URL in the page
      if (strcmp(result, webpage_getURL(page)) == 0){
        ...
        //BEFORE PUTTING MUST CHECK IF IT'S ALREADY INSIDE
          if (hsearch(visitedURLs, searchURL, result, strlen(result)) == NULL && ((hput(visitedURLs, result, result, strlen(result))) == 0)){
